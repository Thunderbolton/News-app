...
app.get('/uk', async (req, res) => {
  const url = `https://newsapi.org/v2/everything?q=(UK OR United Kingdom OR Britain OR England OR Scotland OR Wales OR London)&from=2022-04-24&domains=bbc.co.uk,theguardian.com,dailymail.co.uk&language=en&apiKey=${api_key}`;
...
app.get('/usa', async (req, res) => { 
  const url = `https://newsapi.org/v2/everything?q=(USA OR United States)&from=2022-04-24&domains=cnn.com,axios.com,nbcnews.com&language=en&apiKey=${api_key}`;
13
  const response = await fetch(url)
  .then(res => res.json())  
...
app.get('/canada', async (req, res) => { 
  const url = `https://newsapi.org/v2/everything?q=(Canada OR Toronto OR Vancouver)&from=2022-04-24&domains=ctvnews.ca,globalnews.ca,thestar.com&language=en&apiKey=${api_key}`;
...
app.get('/australia', async (req, res) => { 
  const url = `https://newsapi.org/v2/everything?q=(Australia OR Queensland OR New South Wales)&from=2022-04-24&domains=abc.net.au,9news.com.au,smh.com.au&language=en&apiKey=${api_key}`;
